...
# This file only works as a script run directly
...
# Use a specific time rather than the current time for testing
current_time = datetime.datetime(
    2021, 1, 1, 12, 00, 00, 0, tzinfo=datetime.timezone.utc
)

# reference coordinates for MeerKAT (latitude, longitude, altitude?)
refAnt = (-30.71106, 21.44389, 1035)
wavelength = con.c / current_freq
J2000RefTime = datetime.datetime(2000, 1, 1, 11, 58, 56, tzinfo=datetime.timezone.utc)
...
ObsTime = (
    ...
    + current_time.second / (60.0 * 60.0)
    + current_time.microsecond / (1000000.0 * 60.0 * 60.0)
)
...
for b in BaselineList:
    ...
    Rot = np.array(
        [
            np.cos(refLat) * np.sin(el) - np.sin(refLat) * np.cos(el) * np.cos(azim),
...
br = np.mgrid[
    gridNum - halfLength : gridNum : interval, gridNum - halfLength : gridNum : interval
]

imagesCoord = np.array(
    [
        np.concatenate(
            (np.concatenate((ul[0].T, ur[0].T)).T, np.concatenate((bl[0].T, br[0].T)).T)
        ).flatten(),
        np.concatenate(
            (np.concatenate((ul[1].T, ur[1].T)).T, np.concatenate((bl[1].T, br[1].T)).T)
        ).flatten(),
    ]
)
...
# Testing code: check our output against the golden output
