...
class BeamShape(object):
    ...
    def fit_ellipse(self):
        ...
        for a_tuple in longest_contour:
            contour_ra.append(a_tuple[0])
            contour_dec.append(a_tuple[1])
        mean_ra = sum(contour_ra) / len(longest_contour)
        mean_dec = sum(contour_dec) / len(longest_contour)
        ...
        with open("sanity_check/ellipse_vertices.csv", "w") as f:
            ...
            writer = csv.writer(f)
            writer.writerow(cols)
            for point in ellipse.centered_at_origin().contour(300):
